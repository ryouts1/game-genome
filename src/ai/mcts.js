...
function pickRolloutAction(game, actions, random) {
...
function rollout(game, state, random, maxDepth) {
  ...
  for (let step = 0; step < maxDepth; step += 1) {
    ...
    const chosenAction = pickRolloutAction(game, actions, random);
...
function makeCandidateEntries(root, rootActions) {
  ...
  return [...byActionKey.values()].sort((left, right) => {
...
export function analyzeMcts(game, state, options = {}) {
  ...
  for (let iteration = 0; iteration < iterations; iteration += 1) {
    ...
    const result = rollout(game, simulationState, random, maxRolloutDepth);
    backpropagate(node, result);
  }

  const candidates = makeCandidateEntries(root, rootActions);
  const best = candidates[0];
  const second = candidates[1];
  const exploredActions = candidates.filter((candidate) => candidate.visits > 0).length;
  const rootCoverage = rootActions.length > 0 ? exploredActions / rootActions.length : 0;
  const confidenceGap = second ? best.winRate - second.winRate : best.winRate;
  ...
  return {
    ...
    rootCoverage,
    confidenceGap,
    summary: `iterations ${iterations} / visits ${best.visits} / win-rate ${(best.winRate * 100).toFixed(1)}%`,
  };
}
