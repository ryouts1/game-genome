...
export function evaluateStateDetailed(game, state, rootPlayerId) {
  const status = getGameStatus(game, state);
  const terminalScore = game.evaluation?.terminalScore ?? 100000;

  if (status.isTerminal) {
    const total = status.outcome === 'draw'
      ? 0
      : status.winner === rootPlayerId ? terminalScore : -terminalScore;
  ...
  const weights = buildWeights(game);
  const raw = {
    material: evaluateMaterial(game, state, rootPlayerId),
    mobility: evaluateMobility(game, state, rootPlayerId),
    centerControl: evaluateCenterControl(game, state, rootPlayerId),
    advancement: evaluateAdvancement(game, state, rootPlayerId),
    linePotential: evaluateLineWindows(game, state, rootPlayerId),
  };

  const weighted = {
    material: raw.material * weights.material,
    mobility: raw.mobility * weights.mobility,
    centerControl: raw.centerControl * weights.centerControl,
    advancement: raw.advancement * weights.advancement,
    linePotential: raw.linePotential * weights.linePotential,
  };
...
export function evaluateState(game, state, rootPlayerId) {
  return evaluateStateDetailed(game, state, rootPlayerId).total;
}
