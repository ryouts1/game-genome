...
test('knight-skirmish creates L-shaped jump moves', () => {
  const game = loadFixture('knight-skirmish.json');
  const state = createInitialState(game);
  const actions = generateLegalActions(game, state);

  const move = actions.find((action) => action.from?.x === 1 && action.from?.y === 0 && action.to.x === 2 && action.to.y === 2);
  assert.ok(move);
});
